...
pub struct SimulatedRoute {
    pub merged_amounts: Vec<U256>,      // [baseIn, ..., tokenX, ..., baseOut]
...
pub fn simulate_all_paths_for_token_x(
    ...
    candidate_routes
        ...
        .filter_map(|route| {
            ...
            let buy_amounts = simulate_buy_path_amounts_array(&buy_path, token_x_amount, reserve_cache, token_index)?;
            let sell_amounts = simulate_sell_path_amounts_array(&sell_path, token_x_amount, reserve_cache, token_index)?;
...
/// Pretty-print a merged arbitrage path with amounts and symbols
pub fn print_simulated_route(route: &SimulatedRoute) {
    ...
    println!("  Profit: {} {}", route.profit, route.merged_symbols.first().unwrap_or(&"?".to_string()));
}
