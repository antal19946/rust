...
use revm::interpreter::instructions::i256;
use serde_json::json;
                                        ...
                                        Some("SwapV3") | Some("PancakeSwapV3") => {
                                                ...
                                                parse_swap_v3_event(&event)
                                            {
                                                ...
                                                update_reserve_cache_swap_v3(
                                                    ...
                                                    liquidity,
                                                    tick,
                                                    Some(amount0), // amount0
                                                    Some(amount1), // amount1
                                                    None, // token0
                                                    None, // token1
                                                    &token_index,
                                                    &precomputed_route_cache,
...
fn parse_swap_v3_event(event: &serde_json::Value) -> Option<(H160, U256, U256, i32,I256,I256, H256)> {
    let addr_str = event["address"].as_str()?;
    let sqrt_str = event["sqrt_price_x96"].as_str()?;
    let liq_str = event["liquidity"].as_str()?;
    let tick_str = event["tick"].as_str()?;
    let amount0_str = event["amount0"].as_str()?;
    let amount1_str = event["amount1"].as_str()?;
      let tx_hash_str = event["tx_hash"].as_str()?;
       let tx_hash = H256::from_slice(&hex::decode(tx_hash_str).ok()?);
    // println!(
    //     "[DEBUG] parse_swap_v3_event: addr_str={:?}, sqrt_str={:?}, liq_str={:?}, tick_str={:?} amount0_str={:?}, amount1_str={:?}",
    //     addr_str, sqrt_str, liq_str, tick_str, amount0_str, amount1_str
    // );
    ...
    Some((address, sqrt_price_x96, liquidity, tick, 
        I256::from_dec_str(amount0_str).unwrap_or(I256::zero()), 
        I256::from_dec_str(amount1_str).unwrap_or(I256::zero()),tx_hash))
}
...
fn update_reserve_cache_swap_v3(
    ...
    let (token_x, token_x_amount): (H160, U256) = if let Some(amount0_val) = amount0 {
        if amount0_val < I256::zero() {
            // token0 bought (reserve0 decreased)
            if let Some(pool_data) = reserve_cache.get(&pool) {
                (pool_data.token0, U256::from(amount0_val.unsigned_abs()))
            } else {
                return;
            }
        } else if let Some(amount1_val) = amount1 {
            if amount1_val < I256::zero() {
                // token1 bought (reserve1 decreased)
                if let Some(pool_data) = reserve_cache.get(&pool) {
                    (pool_data.token1, U256::from(amount1_val.unsigned_abs()))
                } else {
                    return;
                }
            } else {
    ...
    // Clone Arc references before moving into async task
    ...
    tokio::spawn(async move {
        ...
        // Declare mutable variables for timing
        let mut after_sim;
        let mut before_tx;
        let mut after_tx;
        let mut tx_hash_str: Option<String> = None;

        if let Some((opportunity, _latency_ms)) = find_arbitrage_opportunity_from_price_tracker(
            &decoded_swap,
            &reserve_cache,
            &token_index,
            &precomputed_route_cache,
...
pub async fn test_arb(
    ...
    let decoded_swap = DecodedSwap {
        tx_hash: H160::zero(),
        pool_address: H160::from_slice(&hex::decode("172fcd41e0913e95784454622d1c3724f546f849").unwrap()),
        token_x: H160::from_slice(&hex::decode("55d398326f99059ff775485246999027b3197955").unwrap()),
        token_x_amount: U256::from_dec_str("63472535314412411159").unwrap(),
        block_number: 0,
        timestamp: chrono::Utc::now().timestamp() as u64,
    };
      let t0 = Instant::now();
    ...
    .await
    {
        println!("Found arbitrage opportunity: {:?}", opportunity);
            after_sim = t0.elapsed().as_micros();
            timings.insert("after_sim_us".to_string(), serde_json::json!(after_sim));

            // Log the opportunity
            log_opportunity_from_price_tracker_test(
                &opportunity,
                latency_ms,
                &reserve_cache,
                // tx_hash,
...
fn log_opportunity_from_price_tracker_test(
    opportunity: &ArbitrageOpportunity,
    latency_ms: u128,
    reserve_cache: &crate::cache::ReserveCache,
    // tx_hash: H256,
